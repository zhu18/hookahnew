class SettleAPILogsController {
  constructor($scope, $rootScope, $http, $stateParams, $uibModal, usSpinnerService, growl) {
    ...
    $scope.search = function (initCurrentPage) {
      ...
      var promise = $http({
        method: 'GET',
        url: $rootScope.site.apiServer + "/api/order/findInvokeStatus",
        params: {
          goodsSn: $stateParams.goodsSn,
          orderSn: $stateParams.orderSn,
          ip: $scope.ip?$scope.ip:null,
          startDate: $scope.startDate ? format($scope.startDate, 'yyyy-MM-dd HH:mm:s') : null,
