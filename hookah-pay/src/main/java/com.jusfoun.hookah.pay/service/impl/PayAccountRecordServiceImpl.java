...
//	FixClientUtil client;
	...
	public void entryPayments(MoneyInOutBo moneyInOutBo) {
		...
		try {
			...
			logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---添加payAccountRecord记录-->操作时间：" + LocalDateTime.now());

			// 添加内部流水记录
			PayTradeRecord payTradeRecord = payTradeRecordService.initPayTradeRecord(moneyInOutBo, payAccountRecord.getId().toString());
			logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---添加payTradeRecord记录-->操作时间：" + LocalDateTime.now());
			...
			logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---获取银行卡信息-->操作时间：" + LocalDateTime.now());
			if(payBankCard == null){
				logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---获取银行卡信息失败-->操作时间：" + LocalDateTime.now());
				throw new HookahException("获取银行卡信息失败");
			}
			...
			paramMap.put("FID_SQSJ", LocalDateTime.now().withNano(0).toString());
			...
			logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---待发送报文信息存到MgMoneyInOutLog数据库-->操作时间：" + LocalDateTime.now());
			...
			AxCallFunc callFunc = new AxCallFunc() {
				public boolean onReply(JFixSess jFixSess, JFixComm jFixComm) {
					try{
						...
						logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---返回报文信息存到MgMoneyInOutLog数据库-->操作时间：" + LocalDateTime.now());
						...
						logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---业务处理--->payTradeRecord修改" + (n > 0 ? "成功" : "失败") + "-->操作时间：" + LocalDateTime.now());
						...
						logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---业务处理--->payAccountRecord修改" + (j > 0 ? "成功" : "失败") + "-->操作时间：" + LocalDateTime.now());
			...
			ResultBean<Map<String, String>> resultBean = fixClient.sendMoneyIn(paramMap, callFunc);
			logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---发送报文-->操作时间：" + LocalDateTime.now());
...
//				if(m != 1){
//					logger.error("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送成功，payAccountRecordMapper修改失败-->操作时间" + LocalDateTime.now());
//				}
				logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送成功-{}-->操作时间：" + LocalDateTime.now(), successMsg);
				logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送成功-->操作时间：" + LocalDateTime.now());
				...
				if(y != 1){
					logger.error("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送失败，payTradeRecordService修改失败-->操作时间" + LocalDateTime.now());
				}
				...
				int m = payAccountRecordMapper.updateByPrimaryKeySelective(payAccountRecord);
				if(m != 1){
					logger.error("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送失败，payAccountRecordMapper修改失败-->操作时间" + LocalDateTime.now());
				}
				logger.info("当前用户入金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送失败-->操作时间：" + LocalDateTime.now());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void ExitPayments(MoneyInOutBo moneyInOutBo) {
		...
		try {
			...
			// 添加外部流水记录 处理中状态
			PayAccountRecord payAccountRecord = initPayAccountRecord(moneyInOutBo, serialNum);
			logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---添加payAccountRecord记录-->操作时间：" + LocalDateTime.now());

			// 如果是提现 就先去扣客户帐 并添加内部流水
			if(moneyInOutBo.getOperatorType() == PayConstants.TradeType.OnlineCash.code){
				logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---提现操作先去扣客户账-->操作时间：" + LocalDateTime.now());
				int n = payAccountService.operatorByType(moneyInOutBo, payAccountRecord.getId());
				if(n != 1){
					logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---客户账扣款失败-->操作时间：" + LocalDateTime.now());
					throw new HookahException("客户账扣款失败");
				}
			}
			...
			filters.add(Condition.eq("payAccountId", moneyInOutBo.getPayAccountID()));
			PayBankCard payBankCard = payBankCardService.selectOne(filters);
			logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---获取银行卡信息-->操作时间：" + LocalDateTime.now());
			if(payBankCard == null){
				logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---获取银行卡信息失败-->操作时间：" + LocalDateTime.now());
				throw new HookahException("获取银行卡信息失败");
			}
			...
			// 待发送报文信息先存到Mongon
			mgMoneyInOutLogService.initMgMoneyInOutLog(moneyInOutBo, paramMap, payAccountRecord.getId());
			logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---待发送报文信息存到MgMoneyInOutLog数据库-->操作时间：" + LocalDateTime.now());
			...
			AxCallFunc callFunc = new AxCallFunc() {
				public boolean onReply(JFixSess jFixSess, JFixComm jFixComm) {
					try{
						...
						resultMap.put("FID_JGSM", jFixSess.getItem(FixConstants.FID_JGSM));
						mgMoneyInOutLogService.updateMgMoneyInOutLog(moneyInOutBo, resultMap, payAccountRecord.getId());
						logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---返回报文信息存到MgMoneyInOutLog数据库-->操作时间：" + LocalDateTime.now());
						...
						if(jFixSess.getCount() > 0){
							...
							if (jFixSess.getCode() > 0){
								...
								String errorMsg = String.format("[fix async error][%s]%s",jFixSess.getItem(FixConstants.FID_CODE), jFixSess.getItem(FixConstants.FID_MESSAGE));
								//todo 失败后处理 如果是提现需要冲账
								payAccountRecord.setTransferStatus(PayConstants.TransferStatus.fail.code);

								if(moneyInOutBo.getOperatorType() == PayConstants.TradeType.OnlineCash.code){
									logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---提现失败冲正客户账-->操作时间：" + LocalDateTime.now());
									MoneyInOutBo moneyReturn = moneyInOutBo;
									moneyReturn.setOperatorType(PayConstants.TradeType.CashREverse.code);
									payAccountService.operatorByType(moneyReturn, payAccountRecord.getId());
								}
							}
						}else{
							String errorMsg = String.format("[fix async error][%s]%s",jFixSess.getItem(FixConstants.FID_CODE), jFixSess.getItem(FixConstants.FID_MESSAGE));
							//todo 失败后处理 修改外部流水状态失败
							payAccountRecord.setTransferStatus(PayConstants.TransferStatus.fail.code);
						}
						...
						payAccountRecord.setUpdateTime(new Date());
						int j = payAccountRecordMapper.updateByPrimaryKeySelective(payAccountRecord);
						logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---业务处理--->payAccountRecord修改" + (j > 0 ? "成功" : "失败") + "-->操作时间：" + LocalDateTime.now());
			...
			ResultBean<Map<String, String>> resultBean = fixClient.sendMoneyOut(paramMap, callFunc);
			logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---发送报文-->操作时间：" + LocalDateTime.now());
			if(resultBean.isSuccess()){
				//发送成功
				//todo 发送成功处理
				String successMsg = "[fix error]" + String.format("[%s]%s",resultBean.getCode() , resultBean.getMsg());
				logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送成功-{}-->操作时间：" + LocalDateTime.now(), successMsg);
				logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送成功-->操作时间：" + LocalDateTime.now());
				...
				int m = payAccountRecordMapper.updateByPrimaryKeySelective(payAccountRecord);
				if(m != 1){
					logger.error("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送失败，payAccountRecordMapper修改失败-->操作时间" + LocalDateTime.now());
				}
				logger.info("当前用户出金操作【PayAccountID】---<" +moneyInOutBo.getPayAccountID()+ ">---报文发送失败-->操作时间：" + LocalDateTime.now());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
