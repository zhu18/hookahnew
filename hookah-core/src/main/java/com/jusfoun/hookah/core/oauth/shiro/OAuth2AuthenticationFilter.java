...
public class OAuth2AuthenticationFilter extends AuthenticatingFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if(!loginUrl.contains(this.redirectUrl)){
            this.setLoginUrl(loginUrl+"&redirect_uri="+this.redirectUrl);
        }
        ...
        Subject s1 = SecurityUtils.getSubject();
        boolean aaa =s1.isAuthenticated();
        Subject s2 = (Subject) request.getAttribute("subject");
        if(s2 != null){
            boolean aaa1 = s2.isAuthenticated();
        }
