...
function addTagsFn() {
	$.ajax({
		type: "get",
		url: host.website + '/goodsLabel/findAll',
...
$('.checkTags-confirmBtn').click(function () {
	...
	if (tags.length > 3) {
		...
		var html = '';
		for (list in tags) {
			html += '<span class="tags-list" labid="' + tags[list].id + '">' + tags[list].name + '<i class="fa fa-times-circle"></i></span>';
		}
		$('.tags-val').html(html);
		$('.tags-box').hide();
		removeTags();
