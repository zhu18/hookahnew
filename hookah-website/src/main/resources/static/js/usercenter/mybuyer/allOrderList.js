      ...
      if(list[i].isDeleted==1){//取消订单状态
          ...
          for (var ii = 0; ii < goods.length; ii++) {
              ...
              if(goods[ii].isDiscussPrice == 1){
                  html += '<td class="text-align-left">x' + goods[ii].goodsNumber +'<br/><br/>'+ '咨询服务参考价:'+ (goods[ii].goodsPrice / 100).toFixed(2) +'元</td>';
              }else {
          ...
          html += '</tbody>';
          html += '</table>';
      }else if(list[i].isDeleted==0 && list[i].payStatus == 2){//已经完成订单
          html += '<table>';
          html += '<thead>';
          html += '<tr>';
          html += '<th class="" style="width: 280px;">' + '订单号:' + list[i].orderSn + '</th>';
      ...
      if (data.code == 1) {
        ...
        if (isOffline == 0) {
         ...
         tempHtml = "<div class='confirmKey'><h4>提取密钥：</h4>" +
                    ...
                    "<h5><span style='margin-left: 15px;'> 已调用次数："+(apiObj.data.totalCount-apiObj.data.hasCount)+"</span><span style='margin-left: 15px;'> 剩余次数："+apiObj.data.hasCount+"</span><a style='color:blue;margin-left: 20px;' href='/usercenter/apiLogs?orderSn="+orderSn+"&goodsSn="+goodsSn+"'>查看详情</a></h5></div>";
          Loading.stop();
